...
def enable_audit(db, redis_client, app: str):
    """
    Monkey-patch database connection to intercept save/delete for audit.
    
    Usage:
        enable_audit(db, redis, "deploy_api")
    """
    original_save = db.save_entity
    original_delete = db.delete_entity
    
    async def audited_save_entity(table, data, **kwargs):
        # Get old value for diff (if update)
        old = None
        entity_id = data.get("id")
        if entity_id:
            try:
                old = await db.get_entity(table, entity_id)
            except:
                pass
        
        # Actual save
        result = await original_save(table, data, **kwargs)
    ...
    async def audited_delete_entity(table, entity_id, **kwargs):
        ...
        # Actual delete
        result = await original_delete(table, entity_id, **kwargs)
    ...
    # Apply patches
    db.save_entity = audited_save_entity
    db.delete_entity = audited_delete_entity
