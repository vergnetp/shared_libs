...
.env hierarchy (highest priority first):
  ...
  2. {service}/.env(.ENV)          (service-specific)
  3. {services}/.env(.ENV)         (all services)
  4. {root}/.env(.ENV)             (shared defaults)
...
def load_env_hierarchy(
    ...
    Priority (highest to lowest):
      ...
      2. Service-specific .env(.ENV) (most specific)
      3. Services directory .env(.ENV)
      4. Root .env(.ENV) (least specific defaults)
    ...
    # Resolve ENV name (e.g. dev / uat / prod)
    try:        
        env_name = get_env()
    except Exception:
        env_name = None
    ...
    def env_variants(dir: Path) -> List[Path]:
        files = [dir / ".env"]
        if env_name:
            files.append(dir / f".env.{env_name}")
        return files

    # Build list of .env files (general → specific)
    env_files = [
        *env_variants(root),
        *env_variants(services_dir),
        *env_variants(svc_dir),
    ]
