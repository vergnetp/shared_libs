...
'''
    ...
    async def backup_postgres(
        ...
        Returns:
            Tuple of (success, data) where data is gzipped SQL bytes on success,
            or error message on failure.
        """
        try:
            client = self._get_http_client()
            response = await client.post(
                f"{self.base_url}/backup/postgres",
                json={
                    "container_name": container_name,
                    "database": database,
                    "user": user,
            ...
            if response.ok:
                ...
                error = response.json_or_none() or {"error": response.text}
                return False, error.get("error", "Backup failed")
        except Exception as e:
            return False, str(e)
    ...
    async def backup_redis(self, container_name: str) -> tuple[bool, bytes | str]:
        """
        Backup Redis database (RDB file).
        
        Args:
            container_name: Container running redis
            
        Returns:
            Tuple of (success, data) where data is RDB bytes on success,
            or error message on failure.
        """
        try:
            client = self._get_http_client()
            response = await client.post(
                f"{self.base_url}/backup/redis",
    ...
    async def backup_mongodb(
        ...
        Returns:
            Tuple of (success, data) where data is gzipped archive bytes on success,
            or error message on failure.
        """
        try:
            client = self._get_http_client()
            ...
            response = await client.post(
                f"{self.base_url}/backup/mongodb",
                json=payload,
                headers={"X-API-Key": self.api_key},
            )
    ...
    async def restore_postgres(
        ...
        Args:
            ...
            backup_data: Gzipped SQL bytes
            database: Database name
            user: Username
            
        Returns:
            AgentResponse with restore result
        """
        try:
            import aiohttp
            
            # Use aiohttp directly for multipart form data
            data = aiohttp.FormData()
            data.add_field('backup_file', backup_data, filename='backup.sql.gz')
            data.add_field('container_name', container_name)
            data.add_field('database', database)
            data.add_field('user', user)
            
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.base_url}/restore/postgres",
                    data=data,
                    headers={"X-API-Key": self.api_key},
    ...
    async def restore_redis(
        ...
        try:
            ...
            data = aiohttp.FormData()
            data.add_field('backup_file', backup_data, filename='dump.rdb')
            data.add_field('container_name', container_name)
            
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.base_url}/restore/redis",
                    data=data,
                    headers={"X-API-Key": self.api_key},
    ...
    async def restore_mongodb(
        ...
        """
        Restore MongoDB from archive backup.
        
        Args:
            container_name: Target container
            backup_data: Gzipped archive bytes
        ...
        try:
            ...
            data = aiohttp.FormData()
            data.add_field('backup_file', backup_data, filename='backup.archive.gz')
            data.add_field('container_name', container_name)
            if database:
                data.add_field('database', database)
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"{self.base_url}/restore/mongodb",
                    data=data,
                    headers={"X-API-Key": self.api_key},
                    timeout=aiohttp.ClientTimeout(total=600),
                ) as response:
