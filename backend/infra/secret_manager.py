...
and Docker secret creation for deployment.
"""
...
class DockerSecretManager:
    """
    Manages Docker secrets for secure runtime secret injection
    """
    
    def __init__(self, secret_manager: SecretManager):
        self.secret_manager = secret_manager
    
    def create_docker_secrets(self, project: str, environment: str, service_config: Dict[str, Any]) -> List[str]:
        ...
        for secret_key in required_secrets:
            ...
            if secret_value:
                # Create Docker secret with project/environment prefix
    ...
    def remove_docker_secret(self, secret_name: str) -> bool:
    ...
    def cleanup_project_secrets(self, project: str, environment: str) -> int:
        ...
        secrets = self.list_docker_secrets()
        
        removed_count = 0
        for secret_name in secrets:
            if secret_name.startswith(prefix):
                if self.remove_docker_secret(secret_name):
                    removed_count += 1
...
def read_docker_secret(secret_name: str) -> Optional[str]:
    """Helper function to read Docker secret from mounted file (for application code)"""
    try:
