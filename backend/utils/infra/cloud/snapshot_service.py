...
class SnapshotService:
    ...
    def ensure_snapshot_stream(
        ...
        try:
            ...
            # Step 6: Remove SSH key from droplet (BEFORE snapshot)
            if remove_ssh_key:
                yield from log("🔑 Removing SSH access from droplet...", step=6)
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        yield from log(f"⚠️ Could not clear SSH keys: {result.stderr}")
                except Exception as e:
                    yield from log(f"⚠️ Failed to clear SSH keys: {e}")
            
            # Step 7: Create snapshot
            yield from log("📸 Powering off droplet for snapshot...", step=7)
            ...
            yield from log("🧹 Cleaning up...", step=8)
            self._delete_droplet(droplet_id)
            ...
            if elapsed - last_status_check >= 60:
                try:
                    ...
                    if status_result.stdout.strip():
                        yield {"type": "log", "message": f"  Status: {status_result.stdout.strip().replace(chr(10), ', ')}"}
                except Exception:
                    pass
