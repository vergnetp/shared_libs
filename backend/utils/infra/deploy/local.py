...
class RemoteDeployer:
    """
    Deploy services to remote servers via SSH.
    
    MVP: Uses SSH to execute docker commands on remote server.
    
    Usage:
        deployer = RemoteDeployer(server_ip="138.68.156.65")
    ...
    def deploy(self, config: DeployConfig) -> DeployResult:
        """
        Deploy service to remote server.
        
        Steps:
        1. Build image locally
        2. Save image to tar
        3. Copy tar to remote
        4. Load image on remote
        5. Run container on remote
        """
        logs = []
        local_deployer = LocalDeployer(log_callback=self._log_callback)
        ...
        try:
            # First build locally
            log(f"Starting deployment to {self.server_ip}...")
            log("")
            log("Step 1/5: Building image locally...")
            build_context, dockerfile_path = local_deployer._prepare_build_context(config)
            
            build_success, build_output = local_deployer._build_image(
                dockerfile_path=dockerfile_path,
                context=build_context,
                tag=config.image_name,
                no_cache=config.no_cache,
                log_callback=log,  # Stream build output
            )
            
            if not build_success:
                return DeployResult(
                    success=False,
                    message=f"Failed to build image: {build_output}",
                    logs=logs,
                )
            ...
            # Save image to tar
            log("")
            log("Step 2/5: Saving image to tar...")
            temp_tar = tempfile.mktemp(suffix=".tar")
            save_result = subprocess.run(
                ["docker", "save", "-o", temp_tar, config.image_name],
                capture_output=True,
                text=True,
            )
            ...
            if save_result.returncode != 0:
                return DeployResult(
                    success=False,
                    message=f"Failed to save image: {save_result.stderr}",
                    logs=logs,
                )
            
            # Get file size for feedback
            tar_size_mb = os.path.getsize(temp_tar) / (1024 * 1024)
            log(f"Image saved ({tar_size_mb:.1f} MB)")
            
            # Copy to remote with progress
            log("")
            log(f"Step 3/5: Uploading to {self.server_ip}...")
            log("(this may take a while for large images)")
            
            # Use subprocess.Popen for real-time progress
            scp_process = subprocess.Popen(
                [
                    ...
                    temp_tar,
                    f"{self.ssh_user}@{self.server_ip}:/tmp/image.tar",
                ],
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
                text=True,
            ...
            for line in iter(scp_process.stdout.readline, ''):
                line = line.rstrip()
                if line and not line.startswith('debug'):  # Skip debug noise
                    if 'Transferred' in line or '%' in line:
                        log(line)
            
            scp_process.wait(timeout=600)  # 10 minute timeout for large images
            ...
            if scp_process.returncode != 0:
                return DeployResult(
                    success=False,
                    message="Failed to copy image to remote server",
                    logs=logs,
                )
            ...
            log("")
            log("Step 4/5: Loading image on remote...")
            self._ssh_exec("docker load -i /tmp/image.tar", stream=True, timeout=300)
            
            # Cleanup tar file (non-fatal - image already loaded)
            try:
                self._ssh_exec("rm -f /tmp/image.tar", timeout=30)
            except Exception as cleanup_err:
                log(f"Warning: cleanup failed (non-fatal): {cleanup_err}")
            
            log("Image loaded")
            
            # Stop existing container
            log("")
            log("Step 5/5: Starting container...")
            self._ssh_exec(f"docker stop {config.container_name} 2>/dev/null || true")
            self._ssh_exec(f"docker rm {config.container_name} 2>/dev/null || true")
            
            # Run container
            env_str = " ".join(f"-e {k}={v}" for k, v in config.env_vars.items())
            run_cmd = (
                f"docker run -d "
            ...
            container_id = self._ssh_exec(run_cmd)
            
            if container_id:
                container_id = container_id.strip()
                container_short = container_id[:12] if len(container_id) >= 12 else container_id
                log(f"Container started: {container_short}")
                ...
                # Poll multiple times like local deployer
                for check_num in range(4):
                    time.sleep(2)
                    
                    # Check both Running state and Status
                    status_output = self._ssh_exec(
                        f"docker inspect --format '{{{{.State.Running}}}} {{{{.State.Status}}}} {{{{.RestartCount}}}}' {config.container_name} 2>/dev/null || echo 'false unknown 0'"
                    )
                    parts = status_output.strip().split()
                    is_running = parts[0].lower() == 'true' if parts else False
                    container_status = parts[1].lower() if len(parts) > 1 else 'unknown'
                    restart_count = int(parts[2]) if len(parts) > 2 else 0
                    
                    log(f"Container status: {container_status} (restarts: {restart_count})")
                    
                    # Success: running with no restarts
                    if is_running and container_status == "running" and restart_count == 0:
                        if check_num >= 1:  # Give it at least 2 checks
                            log("")
                            log(f"✅ Deployment successful!")
                            log(f"Service available at http://{self.server_ip}:{config.port}")
                            return DeployResult(
                                success=True,
                                container_id=container_id,
                                container_name=config.container_name,
                                port=config.port,
                                message=f"Service deployed at http://{self.server_ip}:{config.port}",
                                logs=logs,
                            )
                    
                    # Failure states
                    if container_status in ("restarting", "exited", "dead") or restart_count > 0:
                        break
                
                # Check for failure states
                is_failed = not is_running or container_status in ("restarting", "exited", "dead") or restart_count > 0
                
                if is_failed:
                    log("")
                    log("⚠️ Container failed! Fetching logs...")
                    log("--- Container Logs ---")
                    container_logs = self._ssh_exec(f"docker logs --tail 50 {config.container_name} 2>&1")
                    if container_logs:
                        for line in container_logs.split('\n'):
                            log(line)
                    else:
                    ...
                    return DeployResult(
                        ...
                        container_name=config.container_name,
                        port=config.port,
                        message=f"Container failed (status: {container_status}, restarts: {restart_count}). Check logs above.",
                        logs=logs,
                    )
                ...
                return DeployResult(
                    success=False,
                    message="Failed to start container on remote",
                    logs=logs,
                )
            ...
            local_deployer.cleanup()
    
    def _ssh_exec(self, cmd: str, stream: bool = False, timeout: int = 300) -> str:
    ...
    def logs(self, container_name: str, tail: int = 100) -> str:
        ...
        try:
            return self._ssh_exec(f"docker logs --tail {tail} {container_name}")
        except Exception as e:
            return f"Error: {e}"
