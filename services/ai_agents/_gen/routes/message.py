...
async def create_message(data: MessageCreate, db=Depends(get_db)):
    """Create message."""
    return await crud.create(db, data)


@router.get("/{id}", response_model=MessageResponse)
async def get_message(id: str, db=Depends(get_db)):
    """Get message by ID."""
...
async def update_message(id: str, data: MessageUpdate, db=Depends(get_db)):
    """Update message."""
