...
async def create_thread(data: ThreadCreate, db=Depends(get_db)):
    """Create thread."""
    return await crud.create(db, data)


@router.get("/{id}", response_model=ThreadResponse)
async def get_thread(id: str, db=Depends(get_db)):
    """Get thread by ID."""
...
async def update_thread(id: str, data: ThreadUpdate, db=Depends(get_db)):
    """Update thread."""
...
async def delete_thread(id: str, db=Depends(get_db)):
    """Delete thread."""
